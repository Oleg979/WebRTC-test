...
var app = require('https').createServer(options, function(request, response) {
  ...
  response.write(
    '<title>socketio-over-nodejs</title><h1><a href="' +
      link +
      '">socketio-over-nodejs</a></h1><pre>var socket = io.connect("https://webrtcweb.com:9559/");</pre>'
  );
  response.end();
});
...
io.sockets.on("connection", function(socket) {
  ...
  socket.on("request", data => {
    socket.emit("response", { num: data.num * 2 });
  });
